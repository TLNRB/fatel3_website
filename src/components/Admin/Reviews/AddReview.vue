...
<template>
  ...
  >
    ...
    <div class="flex items-start gap-[1rem] xs:gap-[1.5rem]">
      ...
      <label
        class="h-[34px] w-[100%] overflow-hidden flex flex-col justify-center py-[.5rem] px-[.75rem] border-[1px] border-ltBorder rounded-[7px] cursor-pointer sm:h-[35px]"
        :class="error && !newReview.imgName ? ' border-ltTextNegative' : ''"
      >
        <span
          class="text-[13px] font-light leading-tight sm:text-[.875rem]"
          :class="image ? 'text-TextNormal' : 'text-ltBorderNormal'"
          >{{ image ? image.name : "Choose File" }}</span
        >
